# -*- coding: utf-8 -*-
"""
oauthlib_extras.oauth2.clients.mail_application
~~~~~~~~~~~~~~~~~~~~~~~
...
class MailApplicationClient(Client):

    """A client utilizing the authorization code grant workflow.

    A mail application is a confidential client running on a web
    ...
    Only difference to the web application, is that this is no
    redirection-based flow. Instead of the redirection, the data
    gets sent to the user's mail address.
    """

    def __init__(self, client_id, code=None, **kwargs):
        super(MailApplicationClient, self).__init__(client_id, **kwargs)
        self.code = code
    ...
    def prepare_request_uri(self, uri, redirect_uri=None, scope=None,
        ...
        argument, set to ``code``::
            ...
            >>> client = MailApplicationClient('your_id')
    ...
    def prepare_request_body(self, client_id=None, code=None, body='',
        ...
        return prepare_token_request('authorization_code', code=code, body=body, client_id=self.client_id, **kwargs)

    def parse_request_uri_response(self, secret, state=None):
        ...
        :param secret: Get's sent to you e.g. via mail, and contains the
                    oauth code and state, delimited by the '&' sign.
        ...
        **state**
        ...
        the added benefit of easily extracting parameters from the URI::
            ...
            >>> from oauthlib_extras.oauth2 import MailApplicationClient
            >>> client = MailApplicationClient('your_id')
            >>> secret = 'sdfkjh345&sfetw45'
            >>> client.parse_request_uri_response(secret, state='sfetw45')
            {'state': 'sfetw45', 'code': 'sdfkjh345'}
            >>> client.parse_request_uri_response(secret, state='other')
        ...
        """
        response = parse_authorization_code_response(secret, state=state)
